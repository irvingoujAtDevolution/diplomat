...
{
    /// <c>Box&lt;T&gt;</c>: runs the destructor at the last claim and may take any lease.
    Owned,
...
/// yet — is made here, from both ledgers: at the last claim
/// (<see cref="DropClaim"/>), and for an exclusive view once its scope has ended
/// and no call is in flight (<see cref="ReleaseScopedEdgesIfReady"/>). When the
/// last claim ends, this runs the native destructor before releasing the
...
internal sealed unsafe class RustHandle<T> where T : unmanaged
{
    ...
    internal BorrowLease<T> Lease(BorrowKind kind)
    {
        ...
        catch
        {
            if (borrowed)
            {
                ReleaseBorrowKeepingClaim(kind);
            }
            DropClaim();
            throw;
        }
    ...
    /// Hands the borrow back but keeps the lifetime claim. Returns the version
    /// a view created from this borrow has to check against.
    internal long ReleaseBorrowKeepingClaim(BorrowKind kind)
    {
        long version = _borrows.Release(kind);
        ReleaseScopedEdgesIfReady();
        return version;
    }

    /// Ends the wrapper's own claim; an exclusive view also ends its scope.
    internal void ReleaseOwnerClaim()
    {
    ...
    /// Releases one lifetime claim; the last one runs the destructor, then the edges.
    internal void DropClaim()
    {
        if (!_claims.Release())
        {
            return;
    ...
    private struct BorrowLedger
    {
        ...
        internal void Acquire(BorrowKind kind)
        {
        ...
        internal long Release(BorrowKind kind)
        {
