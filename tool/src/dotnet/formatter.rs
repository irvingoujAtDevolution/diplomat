...
impl<'tcx> DotnetFormatter<'tcx> {
    ...
    /// Format a HIR type's name as a PascalCase C# class / struct / enum
    /// identifier, applying its rename attr. Used wherever a type appears in
    /// generated code by name (declaration sites, return types, params,
    /// `Raw.<Type>.<Method>` qualifications).
    pub fn fmt_type_name(&self, id: TypeId) -> Cow<'tcx, str> {
        ...
        // Most HIR names already arrive PascalCase, but normalize so renames
        // expressed in snake_case still come out clean.
        let cased: Cow<'tcx, str> = renamed.to_upper_camel_case().into();
        escape_keyword(cased)
    }
}
